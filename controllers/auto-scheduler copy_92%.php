...
function getSubjectsForSection($conn, $sectionId, $semester, $academicYear)
{
    ...
    switch ($sectionDept) {
        ...
        case 'BTVTED_Electrical':
            ...
            break;
        case 'BTVTED_All':
            $allowedSubjectDepts[] = 'BTVTED_All';
            break;
        case 'Diploma_Agricultural_Sciences':
            $allowedSubjectDepts[] = 'Diploma_Agricultural_Sciences';
            break;
        case 'BAT_Crops_Production':
            $allowedSubjectDepts[] = 'BAT_Crops_Production';
            break;
        case 'BSA_Agronomy':
            $allowedSubjectDepts[] = 'BSA_Agronomy';
            break;
        default:
...
function getClassrooms($conn, $subjectType, $sectionDept = null)
{
    ...
    if ($subjectType === 'lab') {
        ...
        // All other departments - use any non-GENERAL lab
        $stmt = $conn->prepare("
            SELECT id, room_number, capacity 
            FROM classrooms 
    ...
    if ($examType === 'none') {
        ...
        usort($sections, function ($a, $b) {
            $aPriority = match ($a['year_level']) {
                '1' => 1,
                '2' => 2,
                '3' => 4, // Year 3 has highest priority
                '4' => 3,
                default => 0
            };
        ...
        // Check if all subjects with teachers were scheduled
        foreach ($sections as $section) {
            $sectionId = $section['id'];
            $subjects = getSubjectsForSection($conn, $sectionId, $semester, $academicYear);

            foreach ($subjects as $subject) {
                // Skip subjects without teachers
                if ($subject['teacher_id'] === null) {
                    continue;
                }

                // Calculate required slots the same way as in the scheduling logic
                if ($subject['subject_type'] === 'lecture') {
                    $requiredSlots = ceil(($subject['minutes_per_week'] - 60) / (($subject['minutes_per_week'] - 60) === 120 ? 120 : 60));
                    // Add 1 for the Friday online class
                    $requiredSlots += 1;
                } else {
                    $requiredSlots = ceil($subject['minutes_per_week'] / ($subject['minutes_per_week'] === 120 ? 120 : 180));
                }
                ...
                if ($scheduledSlots < $requiredSlots) {
                    ...
                    // Try to schedule any remaining slots on Friday (for lectures only)
                    if ($subject['subject_type'] === 'lecture') {
                        $day = 'Friday';
                        foreach ($onlineHours as $slot) {
                            $start = date('H:i:s', strtotime($slot[0]));
                    ...
                    if ($subject['subject_type'] === 'pe') {
                        $days = ['Monday', 'Tuesday', 'Wednesday', 'Thursday'];
                        // For PE subjects, try to schedule any remaining slots on other days
                        $daySlots = generateDaySlots($lectureSlots, $lunchBreak, 120);
                        ...
                        foreach ($days as $day) {
                            foreach ($daySlots as $slot) {
                                ...
                                assignSchedule($conn, $subject['id'], $subject['teacher_id'], 0, $day, $start, $end, $semester, $academicYear, $examType, 'pe', $sectionId);
                                $scheduledSlots++;
                                break;
                            }
                        }
                        ...
                        if ($scheduledSlots < $requiredSlots) {
                            header("Location: ../views/admin/admin-scheduler.php?warning=Schedule was complete, but there were some issues. Not enough slots for subject: {$subject['subject_code']} {$subject['subject_name']} for section ID: {$sectionId}. Please adjust minutes_per_week for this subject or add more rooms.");
                            exit();
                        }
                    }
                }
            ...
            // Generate success message with warnings about subjects without teachers
            $message = "Class Schedules generated successfully.";
            if (!empty($subjectsWithoutTeachers)) {
                $message .= " Warning: The following subjects were not scheduled because they don't have assigned teachers: ";
                $subjectDetails = [];
                foreach ($subjectsWithoutTeachers as $subject) {
                    $subjectDetails[] = "{$subject['subject_code']} ({$subject['subject_name']}) in {$subject['section_name']}";
                }
                $message .= implode(", ", $subjectDetails);
            }

            header("Location: ../views/admin/admin-scheduler.php?msg=" . urlencode($message));
            exit();
        }
    } else {
