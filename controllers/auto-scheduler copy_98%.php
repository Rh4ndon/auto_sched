...
if (isset($_POST['submit'])) {
    ...
    if ($examType === 'none') {
        ...
        $labSlots = [
            ['08:00 AM', '10:40 AM'], // 160 min
            ['01:00 PM', '03:40 PM'], // 160 min
            ['08:00 AM', '11:00 AM'], // 180 min
            ['01:00 PM', '04:00 PM'], // 180 min
        ];
        ...
        foreach ($sections as $section) {
            ...
            // Track scheduled subjects for this section
            ...
            foreach ($subjectsWithTeachers as $subject) {
                ...
                // Calculate required slots based on subject type and minutes per week
                if ($subject['subject_type'] === 'lecture') {
                    $requiredSlots = ceil($subject['minutes_per_week'] / 60);
                    $duration = 60; // 60 minutes per session
                } elseif ($subject['subject_type'] === 'pe') {
                    $requiredSlots = 1; // PE is always 120 minutes once per week
                    $duration = 120;
                } else { // lab
                    $requiredSlots = 1; // Labs are scheduled in one block
                    $duration = $subject['minutes_per_week']; // Use the minutes_per_week as duration
                }
                ...
                foreach ($days as $day) {
                    // Skip if we've already scheduled this subject enough
                    if ($scheduledSlots >= $requiredSlots) {
                        break;
                    }

                    // Generate appropriate slots based on subject type
                    if ($subject['subject_type'] === 'lab') {
                        // For labs, we might need to split around lunch
                        $daySlots = [];
                        $totalMinutes = $subject['minutes_per_week'];

                        if ($totalMinutes == 160) {
                            // 160 minutes can be scheduled as one block (2h40m)
                            $daySlots = generateDaySlots($labSlots, $lunchBreak, $totalMinutes);
                        } elseif ($totalMinutes == 180) {
                            // 180 minutes can be scheduled as one block (3h)
                            $daySlots = generateDaySlots($labSlots, $lunchBreak, $totalMinutes);
                        }
                    } elseif ($subject['subject_type'] === 'pe') {
                        $daySlots = generateDaySlots($peSlots, $lunchBreak, 120);
                    } else {
                        $daySlots = generateDaySlots($lectureSlots, $lunchBreak, 60);
                    }
            ...
            // Track which subjects weren't scheduled for this section
            foreach ($subjectsWithTeachers as $subject) {
                if (!isset($sectionSubjectsScheduled[$subject['id']])) {
                    $subjectsNotScheduled[] = [
                        'subject_code' => $subject['subject_code'],
                        'subject_name' => $subject['subject_name'],
                        'section_id' => $sectionId,
                        'section_name' => $section['section_name'] ?? "Section {$sectionId}",
                        'reason' => 'No available time slot or classroom'
                    ];
                }
            }
        ...
        // Prepare a report of scheduling results
        $report = [
        ...
        // Store the report in session to display later
        session_start();
        ...
        foreach ($sections as $section) {
            ...
            foreach ($subjectsWithTeachers as $subject) {
                // Skip lab subjects for exam scheduling
                if ($subject['subject_type'] === 'lab') {
                    /*
                    $subjectsNotScheduled[] = [
                        'subject_code' => $subject['subject_code'],
                        ...
                        'section_id' => $sectionId,
                        'section_name' => $section['section_name'] ?? "Section {$sectionId}",
                        'reason' => 'Lab subjects excluded from exams'
                    ];
                    */
                    continue;
                }
                ...
                $scheduledSlots = 0;
                $duration = 90; // Default duration for exams (90 minutes)
                ...
                // Track if this subject wasn't scheduled
                if (!isset($sectionSubjectsScheduled[$subject['id']])) {
                    $subjectsNotScheduled[] = [
                        'subject_code' => $subject['subject_code'],
                        'subject_name' => $subject['subject_name'],
        ...
        // Prepare a report of exam scheduling results
        $report = [
            'sections_scheduled' => count($sectionsScheduled),
            'subjects_without_teachers' => $subjectsWithoutTeachers,
            'subjects_not_scheduled' => $subjectsNotScheduled
